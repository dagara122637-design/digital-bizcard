...
export const DEFAULT_DATA = {
  ...
  contact: {
    email: 'hello@aetherstudio.kr',
    phone: '02-1234-5678',
    address: '서울 강남구 테헤란로 123, 15F',
  },
  links: [
...
// ── 슬라이드 전환 훅 ─────────────────────────────────────────
const TMPL_ORDER = ['dark','white','gold']
...
export default function EditorPage() {
  ...
  const [me, setMe]           = useState({
    particle:true, glitch:true, transition:true,
    scroll:true, counter:true, hover:true, load:true,
  })

  // ── 저장 상태 ───────────────────────────────────────────────
  const [slug, setSlug]         = useState(null)   // 발급된 slug
  const [saving, setSaving]     = useState(false)
  const [saveMsg, setSaveMsg]   = useState(null)   // 성공/실패 메시지
  ...
  return (
    <div style={{ background:t.bg, minHeight:'100vh', color:t.text, fontFamily:'sans-serif', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ background:t.surface, borderBottom:`1px solid ${t.border}`, padding:'12px 20px',
        ...
        <span style={{ fontSize:10, letterSpacing:'4px', color:t.accent, textTransform:'uppercase', fontWeight:700 }}>
          ✦ DIGITAL CARD PRO
        </span>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {shareUrl && (
            <button onClick={copyLink}
              style={{ padding:'8px 16px', background:'transparent', border:`1px solid ${t.accent}`,
                color:t.accent, fontSize:10, letterSpacing:'2px', textTransform:'uppercase', cursor:'pointer' }}>
              {copied ? '✓ COPIED' : '🔗 LINK'}
            </button>
          )}
          <button onClick={handleSave} disabled={saving}
            style={{ padding:'8px 20px', background:t.accent, border:'none',
              color: tmplKey==='white'?'#fff':'#0a0a0f',
              fontSize:10, letterSpacing:'2px', textTransform:'uppercase',
              cursor: saving?'not-allowed':'pointer', fontWeight:700, opacity: saving?0.7:1 }}>
            {saving ? '저장 중...' : slug ? '✦ 업데이트' : '✦ 저장 & 발급'}
          </button>
        </div>
      </div>
      ...
      {/* ── 저장 메시지 ── */}
      {saveMsg && (
        <div style={{ padding:'10px 20px', fontSize:11, letterSpacing:'1px',
          background: saveMsg.type==='ok' ? `rgba(${t.accentRgb},.12)` : 'rgba(255,80,80,.1)',
          borderBottom:`1px solid ${saveMsg.type==='ok' ? t.accent : '#ff5050'}`,
          color: saveMsg.type==='ok' ? t.accent : '#ff5050', display:'flex', alignItems:'center', gap:12 }}>
          <span>{saveMsg.text}</span>
          {saveMsg.type==='ok' && shareUrl && (
            <span style={{ opacity:.7, fontSize:10 }}>{shareUrl}</span>
          )}
        </div>
      )}
        ...
        {[{k:'editor',l:'✏ 편집'},{k:'preview',l:'👁 미리보기'}].map(v => (
          <button key={v.k} onClick={() => setTab(v.k)}
            style={{ flex:1, padding:'11px',
              background: tab===v.k ? t.surfaceAlt : 'transparent',
              border:'none',
              borderBottom: tab===v.k ? `2px solid ${t.accent}` : '2px solid transparent',
              color: tab===v.k ? t.accent : t.textSub,
              fontSize:10, letterSpacing:'3px', textTransform:'uppercase', cursor:'pointer' }}>
            {v.l}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden', minHeight:0 }}>
        ...
        {/* 편집 패널 */}
        <div style={{ width:'38%', flexShrink:0, overflowY:'auto',
          display: tab==='editor' ? 'flex' : 'none', flexDirection:'column' }}>
          <EditorPanel t={t} data={data} setData={setData} co={co} setCo={setCo} me={me} setMe={setMe} />
        </div>

        {/* 미리보기 */}
        <div style={{ flex:1, overflowY:'auto', display: tab==='preview' ? 'block' : 'flex', flexDirection:'column' }}>

          {/* URL 바 */}
          <div style={{ background:t.surfaceAlt, borderBottom:`1px solid ${t.border}`,
            padding:'9px 18px', display:'flex', alignItems:'center', gap:12, flexShrink:0 }}>
            {shareUrl ? (
              <>
                ...
                {QR_URL && (
                  <img src={QR_URL} alt="QR" width={32} height={32}
                    style={{ border:`1px solid ${t.border}`, flexShrink:0 }} />
                )}
              </>
            ) : (
